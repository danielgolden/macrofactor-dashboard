...
export async function POST(req: NextRequest) {
  try {
    ...
    if (fetchErr) {
      console.error("[import] fetch existing hashes failed:", fetchErr);
      return NextResponse.json(
        { error: `fetch existing hashes failed: ${fetchErr.message} (code ${fetchErr.code})` },
        { status: 500 },
      );
    }
    ...
    if (newEntries.length > 0) {
      ...
      for (let i = 0; i < entryRows.length; i += 500) {
        ...
        if (error) {
          console.error("[import] insert entries batch failed:", error);
          return NextResponse.json(
            { error: `insert entries failed: ${error.message} (code ${error.code})` },
            { status: 500 },
          );
        }
      }
    ...
    if (allErr) {
      console.error("[import] fetch all entries failed:", allErr);
      return NextResponse.json(
        { error: `fetch all entries failed: ${allErr.message} (code ${allErr.code})` },
        { status: 500 },
      );
    }
    ...
    for (let i = 0; i < rows.length; i += 500) {
      ...
      if (error) {
        console.error("[import] upsert foods batch failed:", error);
        return NextResponse.json(
          { error: `upsert foods failed: ${error.message} (code ${error.code})` },
          { status: 500 },
        );
      }
    }
    ...
    if (aggregatedNames.size > 0) {
      ...
      if (staleNames.length > 0) {
        const { error: delErr } = await supabase
          ...
          .in("name", staleNames);
        if (delErr) {
          console.error("[import] delete stale foods failed:", delErr);
          return NextResponse.json(
            { error: `delete stale foods failed: ${delErr.message} (code ${delErr.code})` },
            { status: 500 },
          );
        }
      }
    ...
    return NextResponse.json(
      { error: `Unhandled error: ${msg}`, stack },
      { status: 500 },
    );
  }
}
